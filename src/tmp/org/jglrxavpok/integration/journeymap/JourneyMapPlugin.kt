...
class JourneyMapPlugin(): MoarBoatsPlugin, IWaypointProvider {
    ...
    override fun updateList(player: PlayerEntity) {
        ...
        val gameType = if(player.world.server != null) "sp" /* local game */ else "mp" /* multiplayer game */
        val folder: File = File("journeymap/data/$gameType/", player.world.saveHandler.levelDirectory.name)//FileHandler.getWaypointDir()
